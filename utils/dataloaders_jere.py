...
def create_dataloader(path,
    ...
    with torch_distributed_zero_first(rank):  # init dataset *.cache only once if DDP
        if multichannel:
            dataset = LoadMultiChannelImagesAndLabels(
                path,
                imgsz,
                batch_size,
...
class LoadMultiChannelImagesAndLabels(Dataset):
    ...
    def __init__(self,
        ...
        #DIO KODA KOJI CE MI U IM_FILES SPREMIT STA MI TRIBA - JOS MI NE TRIBAJU MULTI CHANNEL SRANJA REKA BI- TEK PRI LOADANJU IMAGEA        
        try:
            f = []  # image files
        ...
        #TODO: no rect mode idk if thats even used whatafak
    ...
    def check_cache_ram(self, safety_margin=0.1, prefix=''):
        ...
        for _ in range(n):
            #TODO: ucitaj sliku  npy pa onda u cv2 valjda
        ...
        if im is None:  # not cached in RAM
            if False: #TODO: isprogramiraj lipo da ne cita cache
                ...
                if False:
                    ...
                    channels = []
                    #TODO: ne radi ovi for sta bi tribalo
                    for channel in self.hyp['channel_features']:
                        channel_name = list(channel.keys())[0]
                        channel_path = list(channel.values())[0]
                        img_id = f.split('/')[-1].split('.')[0]
                        #TODO: switch ca
                        if channel_name == 'rgb':
                            im_rgb = cv2.imread(f)
                            channels.append(im_rgb)
                        elif channel_name == 'depth':
                            #TODO: implemet dynamic lookup of image extension jpg/png stuff
                            depth_extension = '.png'
                            img_id = f.split('/')[-1].split('.')[0]
                            f_depth = os.path.join(data_folder2, channel_path, img_id + depth_extension)
                            depth_img_grayscale = cv2.imread(f_depth, cv2.IMREAD_GRAYSCALE)
                            depth_img_grayscale = np.expand_dims(depth_img_grayscale, axis=2) # add channel dimension (X,Y) -> (X,Y,1)
...
def verify_image_label(args):
    ...
    #return im_file, lb_file, [123,123], None, nm, 1234, ne, nc, msg
    #return im_file, lb, shape, segments, nm, nf, ne, nc, msg
    try:
        # verify images
        #TODO: PIL ne moze otvorit moje slike triba np.load i onda pribacit u PIL objekt prve 3 dimenzije or smth
        im = Image.open(im_file, formats=None)
        #im = np.load(im_file)
        ...
        nc = 1
        msg = f'{prefix}WARNING ⚠️ {im_file}: ignoring corrupt image/label: {e} for args {args}'
